...
export function TopicLearning() {
  ...
  const [preparingOfflineAI, setPreparingOfflineAI] =
    useState(false);

  const [restoringOfflineAI, setRestoringOfflineAI] =
    useState(false);

  const [offlineReady, setOfflineReady] =
    useState(() => isOfflineAIReady());

  const [offlinePrepared, setOfflinePrepared] =
    useState(() => isOfflineAIPrepared());
  ...
  /* =======================================================
     RESTORE OFFLINE AI AFTER REFRESH
  ======================================================= */

  useEffect(() => {
    let cancelled = false;

    async function restoreCachedAI() {
      /*
       * Already loaded in this browser tab.
       */
      if (isOfflineAIReady()) {
        if (!cancelled) {
      ...
      /*
       * Model was prepared previously.
       *
       * Try restoring it from browser cache.
       */
      if (!cancelled) {
        setRestoringOfflineAI(true);
        setOfflineProgress(0);
        setOfflineError('');
      }

      try {
        const restored = await restoreOfflineAI(
          (progressValue) => {
            if (!cancelled) {
              setOfflineProgress(
        ...
        if (!cancelled) {
          setOfflineReady(restored);
          setOfflinePrepared(restored);
        ...
        console.error(
          'Cached Offline AI restore failed:',
          error
        );
        ...
        if (!cancelled) {
          ...
          setOfflineError(
            error instanceof Error
              ? error.message
              : 'Unable to restore Offline AI.'
          );
        }
      } finally {
        if (!cancelled) {
          setRestoringOfflineAI(false);
        }
      }
    }

    void restoreCachedAI();
  ...
  async function handlePrepareOfflineAI() {
    /*
    ...
    /*
     * First preparation requires internet.
     */
    if (!navigator.onLine) {
      setOfflineError(
        'Connect to the internet first to download the Offline AI model.'
      );
    ...
    try {
      ...
      setOfflineError(
        error instanceof Error
          ? error.message
          : 'Unable to prepare Offline AI.'
      );
    } finally {
      setPreparingOfflineAI(false);
    }
  }
  ...
  const lessons =
    lessonTitles[selectedTopic.id] ?? [];

  const completedLessons =
    progress?.lessonsCompleted ?? 0;

  const mastery =
    progress?.masteryPercentage ?? 0;
...
`;
    ...
    try {
      ...
      if (navigator.onLine) {
        try {
          ...
          if (
            ...
            console.warn(
              'Online EchoTutor unavailable. Trying Offline AI.'
            );
          }
        } catch (onlineError) {
          console.warn(
            'Online EchoTutor failed. Trying Offline AI.',
            onlineError
          );
        }
      }
      ...
      if (!reply.trim()) {
        ...
        /*
         * Offline AI was prepared before,
         * but this page does not currently
         * have the engine loaded.
         */
        else if (isOfflineAIPrepared()) {

          const restored =
            await restoreOfflineAI();

          if (!restored) {
            throw new Error(
              'Offline AI could not be restored from browser cache. Please prepare Offline AI again.'
            );
          }
        ...
        /*
         * Offline AI has never been prepared.
         *
         * IMPORTANT:
         * Do NOT automatically download the model here.
         */
        else {
          throw new Error(
            'Offline AI is not prepared yet. Click "Prepare Offline AI" once while connected to the internet.'
          );
        }
      }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3">
          ...
          <Card className="overflow-hidden h-full min-h-[620px] flex flex-col">
            ...
            <div className="p-4 border border-slate-200 rounded-xl bg-white m-4 mb-0">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-text-secondary mt-1">
                    Download the local AI model once to use EchoTutor without internet.
                  </p>
              ...
              {(preparingOfflineAI ||
                ...
                <div className="mt-3">
                  ...
                  <p className="text-xs text-text-secondary mt-2">
                    ...
                    {preparingOfflineAI
                      ? 'Downloading the AI model. Keep this tab open.'
                      : 'Loading the cached AI model. This should not download the model again.'}
              ...
              {messages.length === 0 && (
                ...
                <div className="text-center py-10">
                  ...
                  <p className="text-sm text-text-secondary max-w-md mx-auto mt-2 leading-relaxed">
                    ...
                    I won't just give you the answer. I'll first understand what you know, identify gaps, and adapt the explanation to you.
            ...
            >
              ...
              <p className="text-[11px] text-text-secondary mt-2">
                EchoTutor uses Qwen to adapt explanations to your understanding.
              </p>
